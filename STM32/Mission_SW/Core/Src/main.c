...
const osThreadAttr_t CommRxTask_attributes = { .name = "CommRxTask",
...
const osMessageQueueAttr_t RxQueue_attributes = { .name = "RxQueue" };
/* Definitions for TxQueue */
osMessageQueueId_t TxQueueHandle;
const osMessageQueueAttr_t TxQueue_attributes = { .name = "TxQueue" };
/* Definitions for SystemStatusQueue */
osMessageQueueId_t SystemStatusQueueHandle;
const osMessageQueueAttr_t SystemStatusQueue_attributes = { .name =
		"SystemStatusQueue" };
/* USER CODE BEGIN PV */
...
int main(void) {
	...
	/* Create the queue(s) */
	/* creation of RxQueue */
	RxQueueHandle = osMessageQueueNew(1, sizeof(MissionInput_t),
			&RxQueue_attributes);

	/* creation of TxQueue */
	TxQueueHandle = osMessageQueueNew(1, sizeof(TxMessage_t),
			&TxQueue_attributes);

	/* creation of SystemStatusQueue */
	SystemStatusQueueHandle = osMessageQueueNew(1, sizeof(SystemStatus_t),
			&SystemStatusQueue_attributes);
	...
	CommRxTaskHandle = osThreadNew(StartCommRxTask, NULL,
			&CommRxTask_attributes);

	/* creation of MissionTask */
	MissionTaskHandle = osThreadNew(StartMissionTask, NULL,
			&MissionTask_attributes);

	/* creation of CommTxTask */
	CommTxTaskHandle = osThreadNew(StartCommTxTask, NULL,
			&CommTxTask_attributes);
...
void StartCommRxTask(void *argument) {
	...
	while (StartReceive() != RX_SUCCESS) {
		vTaskDelay(pdMS_TO_TICKS(10U));
	}

	for (;;) {
		if (Receive() != RX_SUCCESS) {
			continue;
		}
		if (Parse(&rx_message) != PARSE_SUCCESS) {
			continue; /* Destination only: keep it until a Current arrives. */
		}
		if (UpdateData(&rx_message) != UPDATE_SUCCESS
				|| CreateMissionInput(&mission_input) != INPUT_SUCCESS) {
			continue;
		}
		if (osMessageQueuePut(RxQueueHandle, &mission_input, 0U, 0U) == osOK) {
			ConfirmRxMessageDelivered();
		}
		/* If the queue is full, pending destination/start survive. */
	}
	/* USER CODE END 5 */
}
