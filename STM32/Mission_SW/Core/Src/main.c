...
void StartMissionTask(void *argument)
{
  /* USER CODE BEGIN StartMissionTask */
	...
	/* Phase 1: retain the latest monitored status without changing flight. */
	SystemStatus_t system_status = { .communication_status = COMM_ERROR,
			.fuel_status = FUEL_NORMAL };

	for (;;) {
		(void)osMessageQueueGet(SystemStatusQueueHandle, &system_status,
				NULL, 0U);
		if (osMessageQueueGet(RxQueueHandle, &mission_input,
		NULL,
		pdMS_TO_TICKS(50U)) != osOK) {
			continue;
		}
...
void StartMonitorTask(void *argument)
{
  /* USER CODE BEGIN StartMonitorTask */
	...
	for (;;) {
		float current_fuel;
		(void)UpdateCommunicationStatus(GetCommunicationStatus());
		if (GetCurrentFuel(&current_fuel)) {
			(void)CheckFuelStatus(current_fuel);
		}
		/* No valid fuel sample: preserve the last known fuel status. */
		SystemStatus_t status = GetSystemStatus();
		(void)xQueueOverwrite((QueueHandle_t)SystemStatusQueueHandle, &status);
		vTaskDelayUntil(&last_wake_time, pdMS_TO_TICKS(50U));
	}
