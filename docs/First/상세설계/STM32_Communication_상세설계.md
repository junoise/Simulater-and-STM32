...
#### 내부 상태
- 정상 패킷 수신 이력: 최초 false
- 마지막 정상 패킷 수신 시각: 수신 이력이 있을 때만 사용
- COMM_TIMEOUT_MS: TBD ms. Unity 송신 주기, 지연 및 허용 누락을 기준으로 추후 확정
...
- 처리 절차 :
    ...
    6. 패킷 전체 수신과 정의된 형식·타입 및 무결성 검사(체크섬 채택 시 포함)가 모두 성공했을 때만 정상 수신 이력과 마지막 정상 수신 시각을 갱신한다.
    7. 항공기 값의 범위 검사는 Data Manager가 담당한다. DATA_VALID 여부로 정상 통신 수신 시각을 갱신하거나 COMM_OK를 판단하지 않는다.
- 오류 처리 :
...
- 오류 처리 :
    - UART 송신에 실패한 경우 TRANSMIT_FAIL을 반환한다.
    - TX Status를 송신 실패 상태로 갱신한다.
    - 실패한 패킷의 재송신 여부는 Comm TX Task의 정책에 따라 처리한다.
---

#### 3.5 GetCommunicationStatus
- 목적: 마지막 정상 패킷 수신 시각으로 수신 통신의 정상 여부를 조회한다.
- 호출 Task: Monitor Task
- 입력: 없음. 내부 수신 이력과 단조 증가 시간 기준을 사용한다.
- 반환값: CommunicationStatus_t (COMM_OK 또는 COMM_ERROR)
- 처리 절차:
    1. 수신 이력과 마지막 정상 수신 시각을 일관된 값으로 읽는다.
    2. 정상 수신 이력이 없으면 COMM_ERROR를 반환한다.
    3. 현재 시각과 마지막 정상 수신 시각의 차이가 COMM_TIMEOUT_MS 이상이면 COMM_ERROR를 반환한다.
    4. 타임아웃 미만이면 COMM_OK를 반환한다.
- 수신·파싱 실패는 마지막 정상 수신 시각을 갱신하지 않는다. 이후 정상 패킷이 들어오면 복구된다.
- 이 상태는 1차 수신 타임아웃 상태다. 개별 RX/PARSE/TX 실패 기록을 대체하지 않으며 TX 성공도 수신 타임아웃을 해제하지 않는다.
- RTOS tick의 ms 환산 및 카운터 순환을 고려한다. 사용할 시간 API와 COMM_TIMEOUT_MS 수치는 TBD다.
- Comm RX Task의 기록 갱신과 Monitor Task의 조회 사이에 동시 접근 보호가 필요하다. 갱신·조회 양쪽에 적용할 방법은 TBD이며 volatile만으로 해결되었다고 보지 않는다.
- Communication은 System Monitor 함수를 직접 호출하지 않는다. Monitor Task가 조회 결과를 UpdateCommunicationStatus()에 전달한다.
...
5. PARSE_SUCCESS인 경우 정상 수신 시각을 갱신하고 RxMessage_t를 Data Manager에 전달한다.
